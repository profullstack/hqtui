/**
 * Every widget HQTUI ships, one function each, in plain JavaScript.
 *
 * No TypeScript, no build step, no types to import: `node
 * examples/widgets/gallery.js` renders all of them headlessly and prints the
 * result. The library is written in TypeScript but ships compiled JavaScript,
 * so nothing here needs a compiler.
 *
 * The `@widget` / `@end` markers are what hqtui.com/widgets slices to show the
 * code for one widget, which is why a snippet on the site is always a region
 * of a program that runs.
 *
 * Keep each function self-contained: it takes a container and a theme, and
 * nothing else.
 */
...
// @widget label
export function label(ui, theme) {
...
// @widget heading
export function heading(ui, theme) {
...
// @widget badge
export function badge(ui, theme) {
...
// @widget divider
export function divider(ui, theme) {
...
// @widget keyValues
export function keyValues(ui, theme) {
...
// @widget table
export function table(ui, theme) {
  ui.table({
    rows: [
...
// @widget meter
export function meter(ui, theme) {
...
// @widget graph
export function graph(ui, theme) {
...
// @widget sparkline
export function sparkline(ui, theme) {
...
// @widget histogram
export function histogram(ui, theme) {
...
// @widget donut
export function donut(ui, theme) {
  ui.donut({
    segments: [
...
export function tabs(ui) {
  ui.tabs({ tabs: ["1 dashboard", "2 traffic", "3 sessions", "4 network"], active: 1 });
}
// @end
...
export function commandPalette(ui) {
  ui.commandPalette({
    query: "the",
    items: [
...
const EXAMPLES = {
  text, label, heading, badge, divider, keyValues, statusBar,
  table, list, tree, log,
  meter, meters, progress, graph, sparkline, histogram, heatBar, gauge, donut,
  button, checkbox, select, textInput, tabs,
  modal, commandPalette, tooltip,
};
...
for (const [name, fn] of entries) {
  const out = renderToScreen(({ ui, theme }) => fn(ui, theme), { width: 62, height: 12 })
    .text()
    .trimEnd();
  if (out.trim().length === 0) {
    console.error(`FAIL ${name}: rendered an empty screen`);
    blank++;
    continue;
  }
  console.log(`--- ${name}\n${out}`);
}
console.error(`${entries.length - blank}/${entries.length} widget examples rendered`);
