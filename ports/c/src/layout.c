...
int hq_solve(int total,const hq_constraint *items,size_t n,int gap,int *out) {
    if(total<0 || gap<0 || n>4096 || (n && (!items || !out))) return 0;
    if(!n) return 1;
    resolved local[64];
    resolved *p=n<=64 ? local:malloc(n*sizeof(*p));
    if(!p) return 0;
    int64_t a=(int64_t)total-(int64_t)gap*(int64_t)(n-1);
...
int hq_stack(hq_rect r,const hq_constraint *items,size_t n,int horizontal,int gap,hq_rect *out) {
    if(n>4096 || gap<0 || (n && (!out || !items)) || r.width<0 || r.height<0) return 0;
    if(!n) return 1;
    int local[64];
    int *sizes=n<=64 ? local:malloc(n*sizeof(*sizes));
    if(!sizes) return 0;
    if(!hq_solve(horizontal ? r.width:r.height,items,n,gap,sizes)) { if(sizes!=local) free(sizes); return 0; }
